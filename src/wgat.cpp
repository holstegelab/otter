...
void wga_fa_genotyper(const OtterOpts& params, const std::string& fasta, const std::vector<BED>& bed_regions, BS::thread_pool& pool)
{
	...
	pool.parallelize_loop(0, bed_regions.size(), [&pool, &std_out_mtx, &fasta, &bed_regions, &params](const int a, const int b){
		...
		for(int bed_i = a; bed_i < b; ++bed_i){
			...
			std_out_mtx.lock();
			if(params.is_sam) output_fa2sam(params.read_group, local_bed.chr, local_bed.start, local_bed.end, seq, params.read_group, -1, -1, 1, 1, -1, -1);
			else{
...
void wgat(const std::string& input_file, const std::string& bed_file, const OtterOpts& params){
	BS::thread_pool pool(params.threads);

 	std::vector<BED> bed_regions;
 	parse_bed_file(bed_file, bed_regions);

 	if(input_file.substr(input_file.find_last_of(".") + 1) == "bam") {
 		if(params.is_sam){
 			BamInstance bam_inst;
			bam_inst.init(input_file, true);
			for(int i = 0; i < bam_inst.header->n_targets; ++i){
 		...
 		wga_bam_genotyper(params, input_file, bed_regions, pool);
 	} 
 	else {
 		if(params.is_sam) fa2sam(input_file, params.read_group, std::vector<std::string>{});
		wga_fa_genotyper(params, input_file, bed_regions, pool);
 	}
