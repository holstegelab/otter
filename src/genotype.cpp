...
void pairwise_process(const OtterOpts& params, const int& ac_mincov, const int& tc_mincov, const std::vector<BED>& regions, const std::string& bam, const std::vector<std::string>& index2sample, const std::map<std::string,int>& sample2index)
{
	BS::thread_pool pool(params.threads);
	std::mutex stdout_mtx;
	pool.parallelize_loop(0, regions.size(),
		[&pool, &stdout_mtx, &params, &ac_mincov, &tc_mincov, &bam, &regions, &index2sample, &sample2index](const int a, const int b){
			...
			for(int region_i = a; region_i < b; ++region_i) {
				...
				if(iter == nullptr) std::cerr << "(" << antimestamp() << "): WARNING: query failed at region " << region_str << std::endl;
					...
					if(alleles.size() > 1 && is_multi_sample(allele_sample_indeces)){
		    			...
		    			for(int i = 0; i < sample2intervals.size(); ++i){
...
void genotype(const std::string& bam, const std::string& bed, const OtterOpts& params, const int& ac_mincov, const int& tc_mincov)
{
	...
	pairwise_process(params, ac_mincov, tc_mincov, regions, bam, sample_index, sample2index);
}