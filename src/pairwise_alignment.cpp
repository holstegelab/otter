...
DecisionBound otter_find_clustering_dist(const int& width, const double& dinterval, const double& bandwidth, const std::vector<int>& indeces, AlignmentBlock& sequences, wfa::WFAligner& aligner, DistMatrix& distmatrix)
{
	KDE kde(bandwidth);
	...
	for(int i = 0; i < (int)densities.size(); ++i) {
		...
		//std::cerr << (i*dinterval) << '\t' << densities[i] << '\n';
	}
...
void otter_hclust(const bool& ignore_haps, const int& max_alleles, const double& bandwidth, const double& max_tolerable_diff, const double& min_cov_fraction, const int& min_cov_fraction2_l, const double& min_cov_fraction2_f,const std::vector<int>& spannable_indeces, DistMatrix& distmatrix, wfa::WFAligner& aligner, AlignmentBlock& sequences, std::vector<int>& cluster_labels, int& initial_clusters)
{
	...
	if(spannable_indeces.size() == 1) cluster_labels[spannable_indeces[0]] = 0;
		...
		if(max_alleles == 1) {
			...
			int width = int(max_tolerable_diff/0.0025);
			width = width < 1 ? 1 : width;
			DecisionBound dists = otter_find_clustering_dist(width, 0.0025, bandwidth, spannable_indeces, sequences, aligner, distmatrix);
			//std::cerr << dists.dist0 << '\t' << dists.dist1 << '\t' << dists.cut0 << std::endl;
		    ...
		    if(dists.dist1 - dists.dist0 <= max_tolerable_diff) {
		    	...
		    	double dist_final = dists.dist1 == bandwidth ? dists.dist1 : dists.cut0 + bandwidth;
